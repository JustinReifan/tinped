    ...
    @forelse ($channel as $row)
        <div class="col-12 col-md-6 col-lg-4">
            <div class="payment-menu h-100">
                <input type="radio"
                    onclick="inputmetode('{{ $row->id }}','{{ $row->rate }}','{{ $row->rate_type }}')"
                    name="method" id="payment_{{ $row->id }}" value="{{ $row->id }}" data-payment-group="te">
                <label for="payment_{{ $row->id }}" class="payment-item h-100">
                    <div class="info-top">
                        <div>
                            <img src="{{ $row->image }}" alt="" height="25px">
                        </div>
                    </div>
                    <div class="info-bottom text-sm-left">
                        ...
                        <div class="">
                            @if ($row->type_proses == 'otomatis')
                                Dicek otomatis
                            @else
                                Konfirmasi Manual Melalui Whatsapp
                            @endif
                        </div>
                    </div>
                </label>
